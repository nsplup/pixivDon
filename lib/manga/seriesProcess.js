  ...
  return new Promise((done, rej) => {
    const onresponse = async (err, res) => {
      ...
      const newTask = {
        ...
        updateDate, recentUpdatedWorkIds, total, contents,
      }
...
function doTask (newTask, workPath) {
  ...
  return new Promise((done, rej) => {
    ...
    const oldUpdateDate = oldTask.updateDate || 0
    const newUpdateDate = newTask.updateDate
    let shouldUpdate = new Date(oldUpdateDate) < new Date(newUpdateDate)
    ...
    for (let i = 0, len = newContents.length; i < len; i++) {
      ...
      shouldUpdate = shouldUpdate && newTask.recentUpdatedWorkIds.includes(id)
